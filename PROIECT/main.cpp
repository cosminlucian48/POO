...
ostream& operator<<(ostream& out, Player& obj)
{
    out<<" Name: "<<obj.getName()<<"\t"<<" Health: "<<obj.getHP()<<" / "<<obj.getHPMax()<<
        "\tDamage: "<<obj.getDmg()<<"\n Level: "<<obj.getLevel()<<
        "\tExp :"<<obj.getExp()<<" / "<<obj.getExpNext()<<"\tDefense: "<<obj.getDef()<<"\n\n";
    return out;
}
...
void Player::exp_checker(Player &obj)
{
    ...
    {
        obj.setExp(obj.getExpNext()-obj.getExp());
    }
    if(obj == obj.getExpNext())
...
public:
    ...
    inline const int getExp() const { return this->mobExp; }
...
void Game::boss_fight(Player &obj)
{
    ...
    while(action2)
    {
        ...
        cout<<"\n LVL"<<obj.getLevel()<<" Player name: "<<obj.getName()<<"\n\n Player stats:  | HP:"<<obj.getHP()<<"/"<<
            obj.getHPMax()<<"  | EXP:"<<obj.getExp()<<"/"<<obj.getExpNext()<<" |\n\t\t| ATK:"<<obj.getDmg()<<"      | DEF:"<<obj.getDef()<<"     |\n\n";
    ...
    while(action == true)
    {
        ...
        case 1:
            ...
            if (obj.getHP()<=0)
            ...
            else if (mob.getHP()<=0 && fighting==true)
                {
                    ...
                    if (mob.getDoubleExp()==true)
                        mob.setExp(2*mob.getExp());

                    cout<<mob;

                    ++mob;
                    obj.setExp((int)mob.getExp());
                    obj.exp_checker(obj);
                    cout<<"\n You have "<<obj.getExp()<<"/"<<obj.getExpNext()<<" EXP.\n\n";
                    int hp_missing = 0.6*(obj.getHPMax() - obj.getHP());
                    obj.setHP((int)(hp_missing));
            ...
            else if(fighting == false && mobSec.getHP()<=0)
            {
                ...
                if (mob.getDoubleExp()==true)
                    mob.setExp(2*(int)mob.getExp());

                cout<<mobSec;

                obj.setExp(mobSec.getExp());
                obj.exp_checker(obj);
                cout<<"\n You have "<<obj.getExp()<<"/"<<obj.getExpNext()<<" EXP.\n\n";
                if(mob==mobSec)
...
void Game::main_menu(Player &obj)
{
    ...
    switch (choice)
    {
        ...
        case 5:
            ...
            cin>>mob;
            cout<<"\nThe monster this levele are :"<<mobnume<<" ";
